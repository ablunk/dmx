...
using cbsLib::FunctionBaseContext;
...
private:
	...
	typedef int (*fp)(void* self, FunctionBaseContext* context);
	...
	template <typename T, int (T::*M)(FunctionBaseContext* context)>
	struct FunctionCallHelper{
	    static int fCall(void* self, FunctionBaseContext* context){
...
public:
	...
	template <typename T, int (T::*M)(FunctionBaseContext* context)>
	void schedule(void* p, FunctionBaseContext* context){
	    callStack.push(std::make_tuple(FunctionCallHelper<T,M>::fCall,std::unique_ptr<FunctionBaseContext>(context),p));
	}
	// coroutine stack
	std::stack<std::tuple<fp, std::unique_ptr<FunctionBaseContext>,void*>> callStack;

	std::string getName() const {return name;}
	int getPid() const {return pid;}

	// represents the life cycle of a process. Is implemented in derived classes
	virtual int fActions(FunctionBaseContext* context) = 0;
