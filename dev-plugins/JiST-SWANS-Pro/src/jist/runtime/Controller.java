  ...
  private long eventLoop()
  {
    ...
    try
    {
      while(events.size()>0)
      {
        ...
        processEvent();
        numEvents++;
        if (!currentEvent.waiting || currentEvent.waiting && (currentEvent.cont == null || currentEvent.cont.caller == null)) {
  ...
  private void processEvent()
  {
    ...
    try
    {
      ...
      if(call!=null)
      {
        ...
        // waiting?
//        if (currentEvent.waiting) {
//        	waitingEntities.add(currentEvent.ref);
//        }
        // schedule
        if(Main.SINGLE_CONTROLLER)
        {
      ...
      else if(currentCaller!=null)
      {
				...
				//if (!waitingEntities.contains(currentCaller.ref)) {
				if (!currentEvent.waiting) {
					// ONLY return when used as a sleep(x) with x > 0
					// otherwise, the currentCaller is rescheduled by explicit reactivation
					if (Main.SINGLE_CONTROLLER) {
  ...
  //public final Collection<EntityRef> waitingEntities = new HashSet<EntityRef>();
  ...
  public void registerCallEventWaiting(Method method, EntityRef ref, Object[] args)
  {
    Event ev = eventPool.get();
    ev.time = currentSimulationTime;
    ev.method = method;
  ...
  public void disposeEvent(Event ev)
  {
    if(!Main.EVENT_TRACE && !Main.EVENT_LOCATION)
    {
      ...
      ev.method = null;
      ev.waiting = false;
      if(ev.cont!=null && (ev.cont.caller != null && !ev.cont.caller.waiting))
      {
        ev.cont.state = null;
        ev.cont.result = null;
...
//        ev.cont = null;
  ...
  public static Object entityInvocationContWaiting(Method meth, EntityRef ref, Object[] params) throws JistAPI.Continuation, Throwable
  {
    Controller c = getActiveController();
    if(c.isModeRestore())
    {
    ...
    {
      // calling blocking method
      c.registerCallEventWaiting(meth, ref, params);
      return null;
    }
  }
