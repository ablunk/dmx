...
public class WaitQueueReporter extends desmoj.core.report.Reporter {
	 ...
	 *            desmoj.Reportable : The WaitQueue to report about
	 */
	...
	public String[] getEntries() {
		...
		if (source instanceof desmoj.core.advancedModellingFeatures.WaitQueue) {
			...
			// Obs
			entries[3] = Long.toString(wq.getObservations());
			// Qlimit
			...
			// refused
			entries[10] = Long.toString(wq.getMRefused());

			// *** entries of the slave queue
			// Title
			entries[11] = wq.getSlaveQueue().getName();
			// sQorder
			entries[12] = wq.getSQueueStrategy();
			// (Re)set
			entries[13] = wq.getSlaveQueue().resetAt().toString();
			// Obs
			entries[14] = Long.toString(wq.getSlaveQueue().getObservations());
			// Qlimit
			entries[15] = Long.toString(wq.getSlaveQueue().getQueueLimit());
			if (wq.getSlaveQueue().getQueueLimit() == Integer.MAX_VALUE) {
				entries[15] = "unlimit.";
			}
			// Qmax
			entries[16] = Long.toString(wq.sMaxLength());
			// Qnow
			entries[17] = Long.toString(wq.sLength());
			// Qavg.
			entries[18] = Double.toString(wq.sAverageLength());
			// Zeros
			entries[19] = Long.toString(wq.sZeroWaits());
			// avg.Wait
			entries[20] = wq.sAverageWaitTime().toString();
			// refused
			entries[21] = Long.toString(wq.getSRefused());
		} else {
