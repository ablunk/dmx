 ...
 * second in last in first out order. The statistc data of the queue will be
 ...
 * QueueListLifo is associated to and of course, the Entity that is supposed to
 * be enqueued here. This class needs a reference to a subclass of QueueBased to
 * update the queue statistics. It can be used in many kinds of queue
 ...
 * @author based on ideas from Tim Lechler
 * 
 ...
 *
 */
public class QueueListLifo extends QueueList implements
		java.beans.PropertyChangeListener {

	/**
	 * The first link element of the list. It is set to <code>null</code> in
	 * the constructor to indicate that the QueueListLifo is empty.
	 */
	private QueueLink head;

	/**
	 * The last link element of the list It is set to <code>null</code> in the
	 * constructor to indicate that the QueueListLifo is empty.
	 */
	...
	/**
	 * Constructs an empty <code>QueueListLifo</code> with no reference to its
	 * client QueueBased and with the maximum limit of its queue capacity. This
	 * no-arg constructor is necessary to instantiate an object of this class by
	 * calling the java.lang.Class.newInstance() method. The reference to the
	 * QueueBased object making use of this queue-functionality must be provided
	 * later by calling the setQueueBased() method. The limit of the queue
	 * capacity will also be provided later by calling the setQueueLimit()
	...
	public QueueListLifo() {
		...
		// as the QueueListLifo is empty by now set the first and last element
		// of this
	...
	protected boolean checkProcess(SimProcess proc) {
		...
		if (proc == null) // if proc is a null pointer instead of a process
		{
			sendWarning("A non existing process was stored in a queue. "
					...
					"QueueListLifo, Method: first(); called by Class: Stock, Method: "
	...
	/**
	 * Returns true if the given entity is contained in the QueueListLifo, false
	 * otherwise.
	 * 
	...
	boolean contains(Entity e) {
		...
		if (e == null) { // check for nullreference
			sendWarning(
					"Can not check if the given entity is contained in queuelistlifo. "
							+ "Command ignored!",
					"Class: QueueListLifo Method: boolean contains(Entity e).",
	 ...
	 * 
	 * @param e
	 *            Entity : The entity to add to the queuelistlifo
	 */
	public void insert(Entity e) {

		if (e == null) { // check for null reference
			sendWarning(
					"Can not insert Entity in queue. Command ignored!",
					"Class: QueueListLifo Method: void insert(Entity e).",
					"The Entity reference given as parameter is a null reference.",
					"Be sure to only use valid references.");
			return;
		}
		...
		if (contains(e)) { // Entity must not be contained twice in queue
			sendWarning("Can not insert Entity in queue. Command ignored!",
					"Class: QueueListLifo Method: void insert(Entity e).",
		...
		// make a new QueueLink with the given Entity e
		QueueLink newLink = new QueueLink(e, clientQ.presentTime(), this);

		// if queue is empty we can insert the new QueueLink straight away
		if (isEmpty()) {
		...
		// loop through the queue from front to end until pos. or tail found
		for (QueueLink i = head; i != null; i = i.getNext()) {

			// if i <= e the position is found
			if (Entity.isSmallerOrEqual(i.getEntity(), e)) {

				// now insert newLink before position "i"
				// set the pointers for the newLink right
				newLink.setPrev(i.getPrev());
				newLink.setNext(i);

				// set the pointers of "i" right
				i.setPrev(newLink);

				// set the pointers of the QueueLink in front of newLink right
		...
		// now new entity e must be smallest in whole queuelistlifo,
		// so insert as last
	...
	/**
	 * Inserts the given "e" after the position of "which" in the queuelistlifo.
	 ...
	 * inserted and false is returned. Be careful this method might disrupt the
	...
	boolean insertAfter(Entity e, Entity which) {
		...
		if (e == null) { // check for null reference
			sendWarning("Can not insert Entity. Command ignored!",
					"Class 'QueueListLifo' Method: boolean insertAfter(Entity e, "
							+ "Entity which).",
		...
		if (whichLink == null) { // Entity must already be enqueued here
			sendWarning("Can not insert Entity. Command ignored!",
					...
					"The Entity 'which' given as positioning reference is not "
							+ "enqueued in this QueueListLifo.",
					"Make sure the Entity is enqueued by calling "
							+ "method 'QueueListLifo.contains(Entity e)'.");
			return false;
		}
	 ...
	 * queuelistlifo. Returns true if "e" is inserted correctly after "which".
	 * If the queue is empty or the referenced "which" is not contained, the "e"
	 * will not be inserted and false is returned. Be careful this method might
	 * disrupt the priority order of the queue.
	 * 
	 * @return boolean : Is <code>true</code> if inserted correctly,
	 *         <code>false</code> otherwise
	 * @param e
	...
	boolean insertBefore(Entity e, Entity which) {
		...
		if (e == null) { // check for null reference
			sendWarning("Can not insert Entity. Command ignored.",
					"Class 'QueueListLifo' Method: insertBefore(Entity e, "
							+ "Entity which).",
					"The Entity reference 'e' given as parameter is a null "
							+ "reference.",
					"Be sure to only use valid references.");
	 ...
	 * <code>QueueListLifo</code>,<code>false</code> otherwise
	 * 
	 * @return boolean :<code>true</code>, if no elements are inside the
	 *         <code>QueueListLifo</code>,<code>false</code> otherwise
	 */
	...
	/**
	 * Returns the predecessor to the given entity in the queuelistlifo. If
	 ...
	 *         queuelistlifo or <code>null</code> if e has no predecessor in
	 *         the queuelistlifo or entity parameter 'e' itself is not
	 *         contained.
	 * @param e
	 *            Entity : The entity contained in the queuelistlifo whose
	...
	Entity pred(Entity e) {
		...
		if (e == null) { // check for null reference
			sendWarning("Can not return predecessor Entity. Command ignored.",
					"Class: QueueListLifo Method: Entity pred (Entity e).",
					"The Entity reference 'e' given as parameter is a null "
							+ "reference.",
					"Check if Entity 'e' is enqueued using method "
							+ "'QueueListLifo.contains(e)'.");
			return null;
		}
	...
	public void propertyChange(java.beans.PropertyChangeEvent evt) {
		...
		if (evt.getPropertyName() == "avail") {
			...
			if (!isEmpty()) {
				...
				// is the process scheduled alrready?
				if (next.isScheduled()) {
	...
	/**
	 * Removes the given entity from the queuelistlifo. Checks if the given
	 ...
	 * </li>
	 * <li>This queuelistlifo must not be empty, otherwise there's nothing to
	 * remove</li>
	 ...
	 * referencing this queuelistlifo</li>
	 * </ul>
	 ...
	 * 
	 * @return boolean : Is <code>true</code> if the given entity is removed
	 *         successfully from this queuelistlifo, <code>false</code>
	 *         otherwise
	 * @param e
	 *            Entity : The entity to be removed from the queuelistlifo
	 */
	public boolean remove(Entity e) {
		...
		if (e == null) { // check for null reference
			sendWarning(
					...
					"Class: QueueListLifo Method: boolean remove(Entity e).",
					"The Entity reference given as parameter is a null reference.",
					"Be sure to only use valid references.");
			return false;
		}
	...
	/**
	 * Removes the first entity from this QueueListLifo and returns
	 * <code>true</code> if it was removed successfully. If the queuelistlifo
	 * is empty, <code>false</code> is returned.
	 * 
	 * @return boolean : Is <code>true</code>, if the first element has been
	 *         removed successfully, <code>false</code> if the queuelistlifo
	...
	boolean removeFirst() {
		...
		if (isEmpty()) {
			sendWarning(
					...
					"Class: QueueListLifo Method: boolean removeFirst().",
	...
	/**
	 * Removes the last entity from the QueueListLifo and returns
	 * <code>true</code> if it was removed successfully. If the queuelistlifo
	 * is empty, <code>false</code> is returned.
	 * 
	...
	boolean removeLast() {
		...
		if (isEmpty()) {
			sendWarning(
					...
					"Class: QueueListLifo Method: boolean removeLast().",
					"The queue is empty, thus no entity can be removed.",
					"Check if any entity 'e' is enqueued using method "
							+ "'QueueListLifo.contains(e)'.");
			return false; // nothing from nothing leaves nothing
	...
	/**
	 * Returns the successor to the given entity in the queuelistlifo. If there
	 * is no successor or no entity in the queuelistlifo, <code>null</code> is
	 * returned.
	 * 
	 * @return Entity : The entity before the given parameter in the
	 *         queuelistlifo or <code>null</code> if the given entity
	 *         parameter 'e' has no successor in the queuelistlifo or e itself
	 *         is not contained in the queuelistlifo
	 * @param e
	 *            Entity : The entity contained in the queuelistlifo
	 */
	public Entity succ(Entity e) {

		if (e == null) { // check for null reference
			sendWarning("Can not return successing Entity. Command ignored.",
					"Class: QueueListLifo Method: Entity succ (Entity e).",
					"The Entity reference 'e' given as parameter is a null "
							+ "reference.",
					"Check if Entity 'e' is enqueued using method "
							+ "'QueueListLifo.contains(e)'.");
	...
	/**
	 * Returns a string representation of the queuelistlifo. The string is built
	 * by concatenating all string representations of the contained entities,
	 * calling their <code>toString()</code> methods.
	 * 
	 * @return java.lang.String : The string representation of the queuelistlifo
	 */
