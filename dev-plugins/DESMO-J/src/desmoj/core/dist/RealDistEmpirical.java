 ...
 * <code>double</code>. Values produced by this distribution follow an
...
public class RealDistEmpirical extends RealDist {
	 ...
	 * 
	 * @author Tim Lechler
	 * @see desmoj.core.dist.RealDistEmpirical
	 */
	 ...
	 * is done by calling the <code>addEntry(long, double)</code> method to
	 * add values defining the behaviour of the desired distribution.
	 * 
	 * @param owner
	...
	public RealDistEmpirical(Model owner, String name, boolean showInReport,
		...
		values = new java.util.ArrayList<Entry>(); // Initialize List for values
	 ...
	 * <ol>
	 * <li>The first value inserted has to have a cumulative frequency of 0.0
	 * to indicate the lower boundary of the distribution, the last value
	 * inserted has to have a cumulative frequency of 1.0 to indicate the upper
	 * boundary.</li>
	 ...
	 * impossible to interpolate or give a reasonable sample value.</li>
	 * <li>Once this distribution is "initialized", this method does not accept
	 * any more values to be added.</li>
	 * <li>No two values may be equal, since there can not be two diferent
	...
	public void addEntry(double value, double frequency) {
		...
		// first entry must be made with frequency zero
		if (values.isEmpty()) {

			if (frequency == 0) { // valid first entry
				values.add(new Entry(value, frequency));
				return;
			} else { // invalid first entry
				sendWarning(
						"Can't add empirical entry! Command ignored.",
						"RealDistEmpirical : "
								+ getName()
								+ " Method: void addEntry(double value, double frequency)",
						"The given entry is the first entry for this "
								+ "distribution and does not have a "
								+ "frequency of zero (0) : " + frequency,
						"Be sure to add the first entry with a frequency of zero.");
				return; // no proper parameter
			} // frequency must be positive
			...
			if (values.get(values.size()-1).entryValue < value) {
				...
				sendWarning(
						...
						"RealDistEmpirical : "
								+ getName()
								+ " Method: void addEntry(double value, double frequency)",
						"The value for the entry (" + value
								+ ") is lower than the " + "previously given ("
		...
		// frequency must be larger than 0 (because there is already a valid
		// first
		// entry) and must not exceed 1.0
		if ((frequency == 0.0)) {
			sendWarning(
					"Can't add empirical entry! Command ignored.",
					"RealDistEmpirical : "
							+ getName()
							+ " Method: void addEntry(double value, double frequency)",
					"The frequency parameter given is invalid becaus it is "
							+ "zero although an entry with zero has already been made!",
					"Be sure to add an entriy with frequency zero only at the "
	 ...
	 * @see RealDistEmpiricalReporter
	 */
	public desmoj.core.report.Reporter createReporter() {

		return new desmoj.core.report.RealDistEmpiricalReporter(this);
	...
	public double sample() {
		...
		if (!isInitialized) { // no valid entries in Vector
			sendWarning(
		...
		double lowVal = values.get(i-1).entryValue;
		double lowFreq = values.get(i-1).entryFrequency;
		double highVal = values.get(i).entryValue;
		double highFreq = values.get(i).entryFrequency;
