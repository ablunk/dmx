...
public class IntDistUniform extends IntDist {
	 ...
	 * return that given value. In both cases a warning mesage will be produced.
	 * 
	 * @param owner
	...
	/**
	 * Creates the default reporter for the IntDistUniform distribution.
	 * 
	 * @return Reporter : The reporter for the IntDistUniform distribution
	 * @see IntDistUniformReporter
	 */
	public desmoj.core.report.Reporter createReporter() {

		return new desmoj.core.report.IntDistUniformReporter(this);
	...
	public long sample() {
		...
		do {
			if (isAntithetic()) // check if antithetic mode is on
				newSample = (long) Math
						.floor((min + ((max - min + 1) * (1 - randomGenerator
								.nextDouble()))));
			else
				newSample = (long) Math
						.floor((min + ((max - min + 1) * randomGenerator
								.nextDouble())));
		} while (nonNegative && newSample < 0 || newSample == (max + 1)); // get
		// a
		// new
		// sample
		// if
	 ...
	 * makes no sense to set a <code>IntDistUniform</code> object with a
